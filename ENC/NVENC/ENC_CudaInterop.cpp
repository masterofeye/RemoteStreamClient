	...
	namespace ENC{
		namespace NVENC{
			...
			NVENCSTATUS ENC_CudaInterop::InitCuda(unsigned int deviceID)
			{
				...
				string ptx_source;
				char *ptx_path = "C:\\Projekte\\RemoteStreamServer\\ENC\\NVENC\\data\\preproc64_cuda.ptx";
			...
			tenStatus ENC_CudaInterop::Deinitialise(CORE::tstDeinitialiseControlStruct *DeinitialiseControlStruct)
			{
				...
				m_Logger->trace() << "Time to Deinitialise nenEncode_NVIDIA module: " << RW::CORE::HighResClock::diffMilli(t1, t2).count() << "ms.";
#endif
				return enStatus;
			}
			...
			NVENCSTATUS ENC_CudaInterop::ConvertYUVToNV12(EncodeBuffer *pEncodeBuffer, CUdeviceptr cuDevPtr, int height)
			{
...

				{
					...
					copyParamY.WidthInBytes = pEncodeBuffer->stInputBfr.dwWidth;
...

			{
				...
				copyParamU.WidthInBytes = pEncodeBuffer->stInputBfr.dwWidth;
			...
			}
			{
				...
				copyParamV.WidthInBytes = pEncodeBuffer->stInputBfr.dwWidth;
			...
			//TODO: Puffer ausgeben

#define BLOCK_X 32
#define BLOCK_Y 16
			int chromaHeight = height / 2;
			int chromaWidth = pEncodeBuffer->stInputBfr.dwWidth / 2;
...
#undef BLOCK_X
			...
			tenStatus ENC_CudaInterop::Initialise(CORE::tstInitialiseControlStruct * InitialiseControlStruct)
			{
                ...
                m_encodeConfig = *data->pParams;
                data->pParams = nullptr;
				...
				if (m_encodeConfig.nWidth == 0 || m_encodeConfig.nHeight == 0)
				{
					enStatus = tenStatus::nenError;
				...
				m_Logger->debug(" ENC_CudaInterop::Initialise: >>>ENCODING INFO<<<") \
                    ...
                    << "\n size             :" << m_encodeConfig.nWidth << m_encodeConfig.nHeight \
				...
				nvStatus = AllocateIOBuffers(m_encodeConfig.nWidth, m_encodeConfig.nHeight);
				if (nvStatus != NV_ENC_SUCCESS)
				{
					enStatus = tenStatus::nenError;
...
#ifdef TRACE_PERFORMANCE
				RW::CORE::HighResClock::time_point t2 = RW::CORE::HighResClock::now();
				m_Logger->trace() << "Time to Initialise for nenEncode_NVIDIA module: " << RW::CORE::HighResClock::diffMilli(t1, t2).count() << "ms.";
#endif
				return enStatus;
			}
			...
			tenStatus ENC_CudaInterop::DoRender(CORE::tstControlStruct * ControlStruct)
			{
...
#if 0
				nvStatus = ConvertYUVToNV12(pEncodeBuffer, data->pcuYUVArray, m_encodeConfig.nHeight);
				if (nvStatus != NV_ENC_SUCCESS)
				{
...
#else
				// TODO: Remove memcpy. RGB to NV12 conversion should write to the destination buffer directly.
				CUDA_MEMCPY2D copyParamY;
				memset(&copyParamY, 0, sizeof(copyParamY));
				copyParamY.dstMemoryType = CU_MEMORYTYPE_DEVICE;
				copyParamY.dstDevice = pEncodeBuffer->stInputBfr.pNV12devPtr;
				copyParamY.dstPitch = pEncodeBuffer->stInputBfr.uNV12Stride;
				copyParamY.srcMemoryType = CU_MEMORYTYPE_DEVICE;
				copyParamY.srcDevice = data->pcuYUVArray;
				copyParamY.srcPitch = pEncodeBuffer->stInputBfr.uNV12Stride;
				copyParamY.WidthInBytes = pEncodeBuffer->stInputBfr.dwWidth;
				copyParamY.Height = m_encodeConfig.nHeight * 3 / 2;
				CUresult err = cuMemcpy2D(&copyParamY);
				if (err != CUDA_SUCCESS)
				{
					m_Logger->error("ENC_CudaInterop::DoRender: cuMemcpy2D(...) has returned CUDA error ") << err;
					return tenStatus::nenError;
				}
#endif
				...
				nvStatus = m_pNvHWEncoder->NvEncEncodeFrame(pEncodeBuffer, nullptr, m_encodeConfig.nWidth, m_encodeConfig.nHeight, data->pPayload);
				if (nvStatus != NV_ENC_SUCCESS)
				{
				...
				if (pEncodeBuffer != nullptr)
				{
                    ...
                    nvStatus = m_pNvHWEncoder->ProcessOutput(pEncodeBuffer, data->pstBitStream);
					if (nvStatus != NV_ENC_SUCCESS)
					{
